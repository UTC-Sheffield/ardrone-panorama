...
function Panorama(options) {
    ...
    this._folder = options.folder || "/tmp/panorama/";
    this._file = options.file || "output.png";
    ...
    var takePicture = function(callback) {
        ...
        setTimeout(function() {
            mission.client().getPngStream().once('data', function(data) {
                var fileName = this._folder+'pano_' + self._counter++ + '.png';
                // Save the file
    ...
    setTimeout(function() {
        ...
        mission.run(function (err, result) {
            if (err) {
                ...
                console.log("Hugin starting!");
				panoramit.generate({
				  inputPaths: [path.join(this._folder, "*.png")],
				  outputFile: path.join(__dirname, this._file),
				  tempDir: path.join("/tmp/panoramit/"),
